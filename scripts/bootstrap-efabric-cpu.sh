...
# Manually bootstrap/update and enter the canonical ai-research-env portable
# environment on an EFabric CPU Workspace.
#
# Each invocation:
#   1. Clones or fast-forwards ai-research-env to the requested branch.
#   2. Reconciles ai-research-env only when conda-lock.yml changes.
...
#   5. Starts an interactive Bash shell with the environment activated.
#
...
ENV_NAME="${AI_RESEARCH_ENV_CPU_ENV_NAME:-ai-research-env}"
...
usage() {
  cat <<'EOF'
Usage: bootstrap-efabric-cpu.sh [options]

Update ai-research-env, reconcile the canonical portable environment, and launch
an interactive Bash shell with that environment activated.

Options:
  --force-reinstall  Recreate the environment even if the lock hash matches.
  --smoke-test       Run the general environment smoke test before launching.
...
if [[ "${REEXECUTED}" != "1" ]] && \
   [[ -f "${REPO_DIR}/scripts/bootstrap-efabric-cpu.sh" ]] && \
   [[ "${BEFORE_COMMIT}" != "${CURRENT_COMMIT}" ]]; then
  exec env \
    AI_RESEARCH_ENV_BOOTSTRAP_REEXEC=1 \
    AI_RESEARCH_ENV_START_DIR="${START_DIR}" \
    bash "${REPO_DIR}/scripts/bootstrap-efabric-cpu.sh" "${ORIGINAL_ARGS[@]}"
fi

LOCK_FILE="${REPO_DIR}/conda-lock.yml"
if [[ ! -f "${LOCK_FILE}" ]]; then
  echo "Error: portable lockfile not found: ${LOCK_FILE}" >&2
  exit 1
fi

LOCK_HASH="$(sha256sum "${LOCK_FILE}" | awk '{print $1}')"
LOCK_MARKER="${STATE_DIR}/conda-lock.sha256"
...
clean_install() {
  ...
  echo "Installing ${ENV_NAME} from the canonical portable lock ..."
...
if [[ "${NEEDS_RECONCILE}" == true ]]; then
  echo "Portable environment is missing or out of date."
  ...
  echo "Environment now matches conda-lock.yml."
  echo "Environment reconcile time: $((SECONDS - RECONCILE_START)) seconds"
else
  echo "Environment already matches the latest canonical lock."
fi
...
echo
echo "EFabric CPU environment ready."
...
export AI_RESEARCH_ENV_CPU_ENV_NAME="${ENV_NAME}"

SHELL_RC="${STATE_DIR}/efabric-cpu-shell.bashrc"
cat > "${SHELL_RC}" <<EOF
